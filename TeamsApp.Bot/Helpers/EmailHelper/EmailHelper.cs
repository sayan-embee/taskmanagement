...
namespace TeamsApp.Bot.Helpers.EmailHelper
{
    public class EmailHelper : IEmailHelper
    {
        ...
        public async Task<bool> ProcesssEmail_UpdateTask(List<TaskEmailNotificationModel> data)
        {
            try
            {
                if (data != null && data.Any())
                {
                    ...
                    foreach (var item in data)
                    {
                        ...
                        {
                            this._logger.LogError(ex, $"EmailHelper --> ProcesssEmail_UpdateTask() execution failed: {JsonConvert.SerializeObject(item, Formatting.Indented)}");
                            ExceptionLogging.SendErrorToText(ex);
                        }
                    }
            ...
            {
                this._logger.LogError(ex, $"EmailHelper --> ProcesssEmail_UpdateTask() execution failed");
                ExceptionLogging.SendErrorToText(ex);
                return false;
            }
        }
