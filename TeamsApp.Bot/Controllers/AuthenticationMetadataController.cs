            ...
            [FromQuery] string loginHint)
        {
            try
            {
                var consentUrlComponentDictionary = new Dictionary<string, string>
                {
                    ...
                    ["client_id"] = azureOptions.Value.ClientId,
                ...
                var consentUrlPrefix = $"https://login.microsoftonline.com/{azureOptions.Value.TenantId}/oauth2/v2.0/authorize?";
            ...
            {
                logger.LogError(ex, "Error occurred while generating consent URL.");
                throw;
            }
        }
    }
