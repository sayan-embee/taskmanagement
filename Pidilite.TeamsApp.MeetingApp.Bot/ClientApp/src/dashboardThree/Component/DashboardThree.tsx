...
class DashboardThree extends React.Component<Props, State> {
  ...
  popupModule = ( AssignedToEmail : string, MeetingTitleId : number, meetingTypeId : number, taskType : string) => {
    let taskInfo: ITaskInfo = {
        url: `${this.state.OpenAndBehindPop}?assigned=${AssignedToEmail}&login=${this.props.EmailId}&meeting=${MeetingTitleId}&meetid=${meetingTypeId}&tsktyp=${taskType}&fromdate=${this.props.FromDate}&todate=${this.props.ToDate}`,
        title: "Assigned Task",
        height: 350,
        width: 600,
                        ...
                        <TableRow load={load} cells={
                          ...
                          <tr>
                            <td colSpan={12} style={{ padding: "0" }}>
                              <table width={'100%'}>
                                {load.titleWiseData.filter((e:any)=>e.totalTask!==0).map((title: any) => {
                                                ...
                                                return (
                                                  <>
                                                    <tr className='nestedTableRow'>
                                                      ...
                                                      {data3.totalTask !==0 ? 
                                                      <td className='textColorD3' title='View Task' onClick={() => this.popupModule(data3.assignedToEmail,data3.meetingTitleId,data3.meetingTypeId,'TOTAL')} style={{ width: "8%", textAlign: "center", cursor:"pointer" }}>{data3.totalTask}</td> :
                                                      <td style={{ width: "8%", textAlign: "center" , fontSize:"12px"}}>{data3.totalTask}</td>}
