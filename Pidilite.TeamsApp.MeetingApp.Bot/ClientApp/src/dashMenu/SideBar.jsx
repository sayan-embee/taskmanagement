...
// import { GetDivisionsTest, GetMyReporteesTest, GetProfileTest } from '../PidiliteAPI/Pidiliteapi';
import { getUserProfileAPI, GetDivisionsTest, GetMyReporteesTest, GetProfileTest } from '../apis/APIList';
...
const getMyReportees = (email) => {
    GetMyReporteesTest(email).then((res) => {
        if(res.data && res.data.Employee.length > 0){
        }
        else{
  ...
  const getMyDivisions = (email) => {
    ...
    console.log("getMyDivisions----------->",res.data)
    
        if (res.data) {
            let divisions = res.data.filter((el)=>el.DivisionDesc).map((m)=>m.DivisionDesc).toString()
            // let division = "";
            // res.data.forEach((d) => {
            //     if (d && d.divisionName) {
  ...
  const getMyProfile = (email) => {
    GetProfileTest(email).then((res) => {
        ...
        if (res.data) {
            ...
            if (res.data.DivisionDesc.trim() === "Directors Department" &&
                res.data.SalesGroupDesc.trim() === "Directors Department" &&
                ((res.data.pwl >= 1500 || res.data.pwl.trim() === "1500") ||
                    (res.data.Designation.trim() === "Executive Assistant" || res.data.Designation.trim() === "Secretary" || res.data.designation.trim() === "Executive Assistant - Chairman")
                )) {
            }
            else {
            ...
            if (
                res.data.DivisionDesc.trim() === "Human Resource" &&
                res.data.SalesGroupDesc.trim() === "Administration" &&
                (
                    (
            ...
            if (res.data.SalesGroupDesc.trim() === "pilglobinteam") {
            }
            else {
