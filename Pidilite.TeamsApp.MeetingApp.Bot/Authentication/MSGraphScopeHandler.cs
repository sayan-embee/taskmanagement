...
namespace Pidilite.TeamsApp.MeetingApp.Bot.Authentication
{
    using System;
    ...
    public class MSGraphScopeHandler : AuthorizationHandler<MSGraphScopeRequirement>
    {
        ...
        {
            var accessToken = await this.tokenAcquisition.GetAccessTokenForUserAsync(new[] {MeetingApp.Constants.ScopeGroupReadAll });
            var tokenHandler = new JwtSecurityTokenHandler();
            var securityToken = tokenHandler.ReadToken(accessToken) as JwtSecurityToken;
            var claimValue = securityToken.Claims
                .First(claim => claim.Type.Equals(MeetingApp.Constants.ClaimTypeScp.ToString(), StringComparison.CurrentCultureIgnoreCase)).Value;
